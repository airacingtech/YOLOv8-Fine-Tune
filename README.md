...
|---------------------|---------|---------------------------------|
| `finetune.py`       | `finetuning`   | Given YOLO labels, finetunes model for desired time   |
| `sam2_finetune.py`         | `labeling`/ `finetuning`   | Given SAM2 segmasks, formats data into YOLO labels and runs model finetuning|
| `eval.py`     | `evaluation`   | Given a directory of YOLO labels, evaluates performance of given YOLO model   |
| `onnx_inference.py`          | `evaluation`  | Given a video, creates a new video with segmasks generated by YOLO .onnx model   |
...
| `--format_only`     | `flag'  | ❌ No    | If provided, only the data formatting will be run, not model finetuning. |

Template usage:
```bash
python3 sam2_finetune.py /path/to/sam2_data /path/to/data.yaml /path/to/data_destination
